...
module.exports = function auth (options) {
  ...
  function urlmatcher (spec) {
    ...
    _.each(spec, function (path) {
      if (_.isFunction(path)) {
        return checks.push(path)
      }
      if (_.isRegExp(path)) {
        return checks.push(function (req) {
  ...
  function cmd_register (args, done) {
    ...
    useract.register(details, function (err, out) {
      ...
      useract.login({nick: out.user.nick, auto: true}, function (err, out) {
        ...
        if (options.sendemail) {
          mailact.send({
            code: options.email.code.register,
            to: out.user.email,
  ...
  function cmd_create_reset (args, done) {
    ...
    var adjustedArgs = {}
    if (void 0 !== nick) adjustedArgs.nick = nick
    if (void 0 !== email) adjustedArgs.email = email
    ...
    useract.create_reset(args, function (err, out) {
      ...
      if (options.sendemail) {
        mailact.send({
          code: options.email.code.create_reset,
          to: out.user.email,
        ...
        if (service !== 'local') {
          if (!options.service[service].action || options.service[service].action === 'login') {
            ...
            func = function (err, data, info) {
              seneca.act({
                role: 'auth', trigger: 'service-' + options.service[service].action, service: service,
